...
def solution(x, y):
    ...
    # variable to store the number of operations
    operations = 0

    # iterate until either of the numbers equals 1
    while m != 1 or f != 1:

        # base case -> if either m or f is zero then solution is not possible
        if m == 0 or f == 0:
            return "impossible"

        # if one of the numbers is 1, add the other number minus one to operations
        if m == 1:
            operations += f - 1
            break
        elif f == 1:
        ...
        # efficiently reduce the bigger number
        if m > f:
            operations += m // f
            m %= f
        else:
    ...
    # return the number of operations in the form of a string
    return str(operations)


# print(solution('4', '7'))  # expected output: "4"
print(solution('2', '4'))  # expected output: "impossible"
# print(solution('2', '1'))  # expected output: "1"
