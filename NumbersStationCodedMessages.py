...
class Solution(object):
    def solution(self, l, t):

        # initialise two pointers at start of the array acting as a sliding window
        left, right = 0, 0

        # create a variable for storing the sum of the sliding window
        running_sum = l[0]

        # iterate until the window reaches the end of the array
        while right < len(l):

            # if the sum of window meets the target then return the positions of the pointers
            if running_sum == t:
                return [left, right]

            # if sum of window more than target then shift the left pointer
            elif running_sum > t:
                running_sum -= l[left]
                left += 1

            # if sum of window less than target then shift the right pointer
            else:
                right += 1
                if right < len(l):
                    running_sum += l[right]

        # if no solution is found return fixed value
        return [-1, -1]


solution = Solution()

print(solution.solution([6], 6))  # expected output: [0, 0]?
print(solution.solution([1, 2, 3, 4], 15))  # expected output: [-1,-1]
print(solution.solution([4, 3, 5, 7, 8], 12))  # expected output: [0, 2]
print(solution.solution([4, 3, 10, 2, 8], 12))  # expected output: [2, 3]
